...
class App extends Component {
  ...
  decrementCartItemQuantity = id => {
    const {cartList} = this.state
    const updatedDecrementQuantityCartList = cartList.map(eachProduct => {
      if (eachProduct.id === id) {
        return {...eachProduct, quantity: eachProduct.quantity - 1}
      }
    })
  ...
  removeCartItem = id => {
    const {cartList} = this.state
    const updatedCartList = cartList.filter(eachProduct => {
      eachProduct.id !== id
    })
  ...
  updateCartItem = id => {
    const {cartList} = this.state
    const updateQuantityOfCartList = cartList.map(eachProduct => {
      if (eachProduct.id === id) {
        return {...eachProduct, quantity: eachProduct.quantity + 1}
      }
    })
    this.setState({cartList: updateQuantityOfCartList})
  }
